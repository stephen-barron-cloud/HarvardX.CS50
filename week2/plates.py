...
def main():
    # User Input
    potential_plate=input("Enter a desired vanity plate: ")
    
    list_plate_char =[]
    

    var_char = 1
    for char in potential_plate:
        list_plate_char.insert(var_char,char)  
        # print(char)
        var_char +=1
    plate_length = (len(list_plate_char))
    # print(bool_length)
    bool_length = check_length(plate_length)
...
#Checks the length of a string and returns back true if between two values
def check_length(int_length):
    # print(int_length)
    if 2 <= int_length <=6:
        return True
    else: return False
